...
def _from_environment(name: str) -> Optional[str]:
    """Read from the environment, tolerating a differently-cased variable name.

    Environment variables are case-sensitive, so a dashboard entry of
    `fred_api_key` would not satisfy a lookup for `FRED_API_KEY` and the feature
    would report itself unconfigured with no obvious cause. The canonical form
    is upper snake case; other casings are accepted and flagged so the
    ...
    for key, value in os.environ.items():
        if key.lower() == wanted and value:
            logger.warning(
                f"Environment variable {key!r} matched credential {name!r} only "
                f"by case-insensitive comparison; rename it to {name!r}"
            )
            return value
    return None
