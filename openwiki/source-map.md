...
| Change generated-documentation automation | `.github/workflows/openwiki-update.yml`, `.github/openwiki-toolchain/package.json`, `.markdownlint-cli2.yaml`, `openwiki/INSTRUCTIONS.md` | Monday cron, pinned OpenWiki toolchain, `openwiki code --update --print`, watermark-only diff suppression, scoped PR creation, auto-merge arming, authored brief linting | [Deployment and Observability](operations/deployment-and-observability.md) | `npx markdownlint-cli2 openwiki/INSTRUCTIONS.md` when the authored brief changes; review workflow YAML for token scope |
| Change PR-to-wiki cross-linking | `.github/workflows/wiki-crosslink.yml`, `tools/wiki_crosslink.py`, `tests/test_wiki_crosslink.py`, `openwiki/source-map.md` | `pages_for`, same-repo PR guard, exact backticked path matching, root-file basename fallback, unmatched logging, marker comment | [Deployment and Observability](operations/deployment-and-observability.md), [Testing Strategy and Fixtures](testing/strategy-and-fixtures.md) | `python -m pytest tests/test_wiki_crosslink.py -q` |
| Change deterministic documentation gates | `tools/check_wiki_facts.py`, `.github/workflows/lint.yml`, `sources.py`, `tracking.py`, `README.md`, `docs/doctrine.md`, `openwiki/trading/paper-trading-and-live-gate.md` | `near`, `wiki-facts` job, rail and live-threshold assertions, required-check job IDs, README front-door cap | [Deployment and Observability](operations/deployment-and-observability.md), [Testing Strategy and Fixtures](testing/strategy-and-fixtures.md) | `python3 tools/check_wiki_facts.py` |
| Change committed-secret scanning | `.github/workflows/gitleaks.yml` | pinned `gitleaks` version, SHA-256 tarball verification, full-history checkout, `/tmp/gitleaks git . --no-banner --redact` | [Deployment and Observability](operations/deployment-and-observability.md), [Configuration and Secrets](operations/configuration-and-secrets.md) | Workflow review; the CI job is the consumer-facing gate |
| Change docs hub notification | `.github/workflows/notify-wiki-hub.yml`, `AGENTS.md` | `docs-updated` repository dispatch, `HUB_DISPATCH_TOKEN`, docs path filters, chat-lane runbook | [Deployment and Observability](operations/deployment-and-observability.md) | Workflow review; no local network smoke without the GitHub secret |
